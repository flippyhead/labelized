= labelized
A simple Ruby library for labeling things. Similar to and inspired by acts_as_taggable_on but more flexible. Labelized allows you to control every aspect of label items, including
the label to labeled relation. Labels can be organized into label sets. Labelized takes the approach of mixing in functionality with configuration.
...
Labelized doesn't assume much about your model space. You mix in the appropriate labelized module and configure a few options. 
...
== A Simple Example
  ...
  # Include the LabelizedConcern and call labelized on a class you want to be label-able 
  class Thing
